...
stdenv.mkDerivation rec {
  ...
  configureFlags = [
    ...
    ++ stdenv.lib.optional stdenv.buildPlatform.isCygwin "--build=x86_64-unknown-cygwin";
  ...
  postInstall = ''
    for i in $out/bin/*; do
      wrapProgram $i \
        ...
        ${stdenv.lib.optionalString stdenv.hostPlatform.isCygwin "--prefix PATH : $out/lib/expect${version}"}
    done
  '';
