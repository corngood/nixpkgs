          ...
          tests =
            ...
            // lib.optionalAttrs (editor) (
              let
                ...
                project-export = stdenvNoCC.mkDerivation {
                  ...
                  buildPhase = ''
                    ...
                    ln -s "${pkg.export-template}"/share/godot/export_templates "$HOME"/.local/share/godot/
                ...
                project-export-bin = stdenvNoCC.mkDerivation {
                  name = "${pkg.name}-project-export-bin";
                  ...
                  buildPhase = ''
                    ...
                    godot${suffix} --headless -s create-scene.gd
              ...
              in
              {
                inherit project-export project-export-bin;

                project-runs = runCommand "${pkg.name}-project-runs" { } ''
                  (
                    set -eo pipefail
                    HOME=$(mktemp -d)
                    "${project-export}"/bin/test --headless | tail -n1 | (
                      read output
