...
let
  ...
  inherit (callPackage ./hooks {
    inherit dotnet-sdk dotnet-test-sdk disabledTests dotnet-runtime runtimeDeps buildType;
    runtimeId =
      if runtimeId != null
      then runtimeId
  ...
  buildInputsWithoutDeps = args.buildInputs or [] ++ [
    sdkDeps
  ] ++ projectReferences;

  buildInputs = buildInputsWithoutDeps ++ [
    _nugetDeps
  ];

  nuget-packages = symlinkJoin {
    name = "${name}-nuget-packages";
    paths = map (p: "${toString (lib.getDev p)}/share/nuget/packages") buildInputs;
  };
in
stdenvNoCC.mkDerivation (args // {
  ...
  passthru = {
    inherit sdkPackages sdkDeps _nugetDeps nuget-packages;
    ...
    fetch-deps =
      ...
      writeShellScript "fetch-${pname}-deps" ''
        ...
        for input in ${toString (map lib.getDev buildInputsWithoutDeps)}
        do
        ...
        nuget-to-nix "$tmp/nuget_pkgs" >> "$tmpFile"
        mv "$tmpFile" "$depsFile"
